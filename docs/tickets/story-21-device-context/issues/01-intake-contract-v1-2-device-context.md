...
"Amendments → v1.2 (planned)" — the spec stays in `reports-inbox/` permanently as the
...
**Status:** ready-for-agent — grilled and signed off by the developer 2026-08-29
...
- [ ] Migration V7: `reports.os VARCHAR(200) NULL` · `reports.browser VARCHAR(200) NULL`
      · `reports.device_model VARCHAR(200) NULL`. Additive only — no `ALTER` of any
      existing column, nothing dropped.
- [ ] Intake accepts `context.os` / `context.browser` / `context.deviceModel`: each
      ...
      worklog's).
- [ ] Field semantics (the Largata-side build reads this): `os` = OS name **and**
...
- [ ] `GET /api/reports` and both intake responses (201 + idempotent 200 replay) carry
      the three fields; screenshots, statuses, notes, idempotency, both auth schemes
      untouched.
- [ ] Contract tests in `IntakeEndpointTest` (the cross-repo pin): with all three ·
...
- [ ] Inbox renders them as **one combined Device row** in the detail metadata block —
...
- [ ] Live `bootRun` against the compose Postgres confirms V7 actually applied (standing
      silent-Flyway lesson — the test suite alone doesn't prove autoconfigured migration).
- [ ] Manual UI checklist (standing no-automated-e2e decision): a v1.2-shaped report
...
- [ ] Docs: spec wire-contract section edited in place to v1.2 + the "planned" amendment
